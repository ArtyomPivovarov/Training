...
const topThreeWords = text => {
    ...
    return Array.from({ length: 3 }, () => {
        let maxFreqWord = { word: '', freq: -Infinity };
        for (const [ word, freq ] of Object.entries(stats)) {
            if (freq <= maxFreqWord.freq) continue;
            maxFreqWord = { word, freq }
        }
        delete stats[maxFreqWord.word];
        return maxFreqWord.word;
    }).filter(el => !!el)
};
